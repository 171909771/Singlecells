...
# 找cluster
anchors <- ScaleData(anchors, verbose = FALSE)
anchors <- RunPCA(anchors, verbose = FALSE)
anchors <- RunUMAP(anchors, dims = 1:50)

anchors <- FindNeighbors(anchors, reduction = "pca", dims = 1:50)
anchors <- FindClusters(anchors, resolution = 1.5)

p1 <- DimPlot(anchors, reduction = "umap", group.by = "orig.ident")
p2 <- DimPlot(anchors, reduction = "umap", label = F, repel = TRUE)
p1 + p2
```
