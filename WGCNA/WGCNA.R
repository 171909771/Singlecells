...
library(reshape2)
matr1=read.csv("tmp.txt",sep = "\t",header = F)
# switch data to matrix
fpkm <- dcast(matr1,formula = V2~V1)
...
if(T){
  ...
  datExpr <- t(fpkm) #if do, the below codes is not available
...
## 首先是一步法完成网络构建 this function includes correlation
# deepSplit means percise; pamRespectsDendro T means that the merged branch contains different branch colors;  pamRespectsDendro F means that grey
...
# calculate correlation between groups and modules
  ...
  png("step5-Module-trait-relationships1.png",width = 800,height = 1200,res = 120)
  par(mar = c(6, 8.5, 3, 3));
...
if(T){
  ...
  # calculate the T test of pairwise
